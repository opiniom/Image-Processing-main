...
def _apply_group_mean_3x3(u_windows, total_normals, thresh, dir_indices, prev_conds):
    """2단계: 방향성 그룹(Group Mean) 필터 로직 - newgruop.txt 설계안 적용"""
    ...
    if np.any(cond):
        ...
        with np.errstate(all='ignore'):
            win_mean = np.nanmean(c_win_f, axis=1) # (N_cond,)
        ...
        for d_idx in dir_indices:
            ...
            with np.errstate(all='ignore'):
                d_m = np.nanmean(d_w_f, axis=1) # (N_cond,)
            
            # Step 4/5: 3x3 평균과 가장 가까운 그룹의 평균 선택
            valid = ~np.isnan(d_m)
...
def amshf_filter(A, return_route=False, return_stats=False,
    ...
    # newgruop.txt 설계안에 따른 그룹 인덱스 [상, 우, 하, 좌]
    dir_indices_3 = [
        [0, 1, 2], # Top
        [2, 5, 8], # Right
        [6, 7, 8], # Bottom
    ...
    while True:
        ...
        # 2. 3x3 방향성 그룹(Group Mean) 필터 연산 (newgruop.txt 로직)
